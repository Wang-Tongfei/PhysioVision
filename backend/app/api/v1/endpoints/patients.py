...
def list_patients(clinic_id: int = None, db: Session = Depends(get_db)):
    q = db.query(Patient)
    if clinic_id:
        q = q.filter(Patient.clinic_id == clinic_id)
    return q.all()
...
def patient_sessions(patient_id: int, db: Session = Depends(get_db)):
    rows = (
